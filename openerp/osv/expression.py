            ...
            # Applying recursivity on field(one2many)
                ...
                if right is not False:
                    if isinstance(right, basestring):
                        ...
                        domain = column._domain or []
                        if callable(domain):
                            domain = domain(model)
                        ids2 = [x[0] for x in comodel.name_search(cr, uid, right, domain, op, context=context, limit=None)]
                        if ids2:
                ...
                if operator in HIERARCHY_FUNCS:
                    ...
                    if right is not False:
                        if isinstance(right, basestring):
                            ...
                            if callable(domain):
                                domain = domain(model)
                            res_ids = [x[0] for x in comodel.name_search(cr, uid, right, domain, op, context=context, limit=None)]
                            if res_ids:
                                operator = 'not in' if operator in NEGATIVE_TERM_OPERATORS else 'in'
                        else:
