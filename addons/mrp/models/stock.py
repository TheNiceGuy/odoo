...
class StockMove(models.Model):
    ...
    def _action_explode(self, move):
        ...
        bis = bom_obj.sudo()._bom_find(product_id=move.product_id.id, properties=property_ids)
        ...
        if bis and bom_point.type == 'phantom':
            ...
            res = bom_obj.sudo()._bom_explode(bom_point, move.product_id, factor, property_ids)
            ...
            for line in res[0]:
                ...
                if product.type != 'service':
                    ...
                    mid = move_obj.browse(move.id).copy(default=valdef)
                    to_explode_again_ids.append(mid.id)
                else:
                    if prod_obj.need_procurement():
                        valdef = {
                        ...
                        if move.procurement_id:
                            proc = proc_obj.browse(move.procurement_id.id).copy(default=valdef)
                        else:
            ...
            if not move.split_from and move.procurement_id:
                ...
                if len(moves) == 1:
                    proc_obj.browse(move.procurement_id.id).write({'state': 'done'})
            ...
            if processed_ids and move.state == 'assigned':
                # Set the state of resulting moves according to 'assigned' as the original move is assigned
                abc = list(set(processed_ids) - set([move.id]))
                move_obj.browse(list(set(processed_ids) - set([move.id]))).write({'state': 'assigned'})
            #delete the move with original product which is not relevant anymore
            move_obj.browse(move.id).sudo().unlink()
...
class StockWarehouse(models.Model):
    ...
    def _get_all_products_to_resupply(self, cr, uid, warehouse, context=None):
        res = super(StockWarehouse, self)._get_all_products_to_resupply(cr, uid, warehouse, context=context)
