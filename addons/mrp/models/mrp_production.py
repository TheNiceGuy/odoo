...
class MrpProduction(models.Model):
    ...
    move_raw_ids = fields.One2many(
        ...
        oldname='move_lines',  # + move_lines2 ?
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    move_finished_ids = fields.One2many(
        'stock.move', 'production_id', 'Finished Products',
        # domain=[('state', 'in', ('done', 'cancel'))],
        # oldname='move_created_ids + move_created_ids2'  # TDE: note sure
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
...
class MrpProductionWorkcenterLine(models.Model):
    ...
    time_ids = fields.One2many(
        'mrp.workcenter.productivity', 'workorder_id')  # TDE FIXME: learn how to name fields
...
class MrpProductionWorkcenterLineTime(models.Model):
    # TDE: move to mrp_workcenter, bande de trous de biche
...
class InventoryMessage(models.Model):
    # TDE CLEANME: this is probably completely useless
