...
class MrpProduction(models.Model):
    ...
    def _get_progress(self):
        ...
        for mrp_production in self:
            if mrp_production.product_qty:
                ...
                mrp_production.progress = done / mrp_production.product_qty * 100
    ...
    def product_id_change(self):
        """ Finds UoM of changed product. @param product_id: Id of changed product."""
    ...
    def bom_id_change(self):
        """ Finds routing for changed BoM. @param product: Id of product."""
        if not self.bom_id:
            self.routing_id = False
    ...
    def _prepare_lines(self, production, properties=None):
        ...
        if not bom_point:
            bom_id = bom_obj._bom_find(product_id=production.product_id.id, properties=properties)
            if bom_id:
                bom_point = bom_obj.browse(bom_id)
                self.bom_id = bom_id
                self.routing_id = bom_point.routing_id.id or False
        ...
        return bom_obj._bom_explode(bom_point, production.product_id, factor / bom_point.product_qty, properties, routing_id=production.routing_id.id)
    ...
    def action_production_end(self):
        ...
        self.env['procurement.order'].search([('production_id', 'in', [self.id])]).check()
    ...
    def _calculate_qty(self, production, product_qty=0.0):
        ...
        if not product_qty:
            product_qty = uom_obj._compute_qty(production.product_uom.id, production.product_qty, production.product_id.uom_id.id) - produced_qty
        production_qty = uom_obj._compute_qty(production.product_uom.id, production.product_qty, production.product_id.uom_id.id)

        scheduled_qty = OrderedDict()
        for scheduled in production.product_lines:
            if scheduled.product_id.type == 'service':
                continue
            qty = uom_obj._compute_qty(scheduled.product_uom.id, scheduled.product_qty, scheduled.product_id.uom_id.id)
        ...
        for product_id in scheduled_qty.keys():
            ...
            for move in production.move_lines:
                ...
                quants = self.env['stock.quant'].quants_get_prefered_domain(move.location_id, move.product_id, q, domain=[('qty', '>', 0.0)], prefered_domain_list=[[('reservation_id', '=', move.id)]])
    ...
    def action_produce(self, production_id, production_qty, production_mode, wiz=False):
        ...
        production_qty_uom = uom_obj._compute_qty(production.product_uom.id, production_qty, production.product_id.uom_id.id)
        ...
        if production_mode == 'consume_produce':
            ...
            for produce_product in production.move_created_ids:
                ...
                new_moves = stock_mov_obj.action_consume(qty, location_id=produce_product.location_id.id, restrict_lot_id=lot_id)
                stock_mov_obj.browse(new_moves).write({'production_id': production_id})
                ...
                if not float_is_zero(remaining_qty, precision_rounding=precision):
                    ...
                    extra_move_id = stock_mov_obj.copy(produce_product.id, default={'product_uom_qty': remaining_qty,
        ...
        if not production.move_created_ids and production.move_lines:
            for move in production.move_lines:
                stock_mov_obj.browse(move).action_cancel()
        ...
        for wc_line in production.workcenter_lines:
            ...
            if wc.costs_journal_id and wc.costs_general_account_id:
                ...
                if value and account:
                    amount += value
                    # we user sudo() as we do not garantee an mrp user
    ...
    #TODO : Remove this method not longer used.
    ...
    def _make_production_produce_line(self, production):
        procs = self.env['procurement.order'].search([('production_id', '=', production.id)])
        procurement = procs and\
            self.env['procurement.order'].browse(procs[0]) or False
        data = {
    ...
    def _get_raw_material_procure_method(self, product, location_id=False, location_dest_id=False):
        ...
        if location_id and location_dest_id:
            pulls = self.env['procurement.rule'].search([('route_id', 'in', [record.id for record in routes]), ('location_id', '=', location_dest_id), ('location_src_id', '=', location_id)], limit=1)
            if pulls:
        ...
        if mto_route in [record.id for record in routes]:
    ...
    def _create_previous_move(self, move_id, product, source_location_id, dest_location_id):
        ...
        src_loc = loc_obj.browse(source_location_id)
        dest_loc = loc_obj.browse(dest_location_id)
        code = stock_move.get_code_from_locs(move, src_loc, dest_loc)
        if code == 'outgoing':
            check_loc = src_loc
        else:
            check_loc = dest_loc
        warehouse = loc_obj.get_warehouse(check_loc)
        domain = [('code', '=', code)]
        if warehouse:
            domain += [('warehouse_id', '=', warehouse)]
        types = self.env['stock.picking.type'].search(domain)
        move = stock_move.copy(move_id, default={
            'location_id': source_location_id,
            'location_dest_id': dest_location_id,
            'procure_method': self._get_raw_material_procure_method(product, location_id=source_location_id, location_dest_id=dest_location_id),
    ...
    def _make_consume_line_from_data(self, production, product, uom_id, qty, uos_id, uos_qty):
        ...
        move_id = self.env['stock.move'].create({
            ...
            'company_id': production.company_id.id,
            'procure_method': prev_move and 'make_to_stock' or self._get_raw_material_procure_method(product, location_id=source_location_id, location_dest_id=destination_location_id), #Make_to_stock avoids creating procurement
