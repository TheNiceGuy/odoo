...
class ProcurementOrder(models.Model):
    ...
    def _prepare_mo_vals(self, procurement):
        ...
        if procurement.bom_id:
            ...
            properties = [x.id for x in procurement.property_ids]
            bom_id = bom_obj.with_context(dict(force_company=procurement.company_id.id))._bom_find(product_id=procurement.product_id.id, properties=properties)
    ...
    def make_mo(self):
        ...
        for procurement in self:
            if self.check_bom_exists():
                ...
                produce_id = production_obj.sudo().with_context(dict(force_company=procurement.company_id.id)).create(vals)
                ...
                production_obj.signal_workflow('button_confirm')
            else:
