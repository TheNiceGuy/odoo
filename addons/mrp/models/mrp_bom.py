...
class MrpBom(models.Model):
    ...
    def _bom_explode(self, bom, product, factor, properties=None, level=0, routing_id=False, previous_products=None, master_bom=None):
        ...
        for bom_line_id in bom.bom_line_ids:
            if bom_line_id.date_start and bom_line_id.date_start > today_date or \
                bom_line_id.date_stop and bom_line_id.date_stop < today_date:
                    continue
    ...
    @api.one
    def unlink(self):
        if self.env['mrp.production'].search([('bom_id', 'in', [self.id]), ('state', 'not in', ['done', 'cancel'])]):
...
class MrpBomLine(models.Model):
    ...
    def _get_child_bom_lines(self):
        ...
        for bom_line in self:
            ...
            if bom_id:
                ...
                bom_line = [bom.id for bom in child_bom.bom_line_ids]
            else:
                bom_line = False
    ...
    def onchange_product_id(self):
        ...
        if self.product_id.uos_id.id:
            ...
            self.product_uos = self.uos_id.id
