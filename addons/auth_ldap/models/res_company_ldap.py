...
class CompanyLDAP(models.Model):
    ...
    def get_ldap_dicts(self):
        ...
        res = ldaps.read(['id','company','ldap_server','ldap_server_port','ldap_binddn','ldap_password','ldap_filter','ldap_base','user','create_user', 'ldap_tls'], load='normal')
        return res
    ...
    def get_or_create_user(self, conf, login, ldap_entry):
        ...
        if res:
            ...
            if conf['user']:
                values['active'] = True
                user_id = SudoUser.browse(conf['user']).copy(default=values).id
            else:
