...
class HrContract(models.Model):
    ...
    tds = fields.Float(string='TDS', digits_compute=dp.get_precision('Payroll'), help='Amount for Tax Deduction at Source')
    driver_salay = fields.Boolean(string='Driver Salary', help='Check this box if you provide allowance for driver')
    medical_insurance = fields.Float(string='Medical Insurance', digits_compute=dp.get_precision('Payroll'), help='Deduction towards company provided medical insurance')
    voluntary_provident_fund = fields.Float(string='Voluntary Provident Fund (%)', digits_compute=dp.get_precision('Payroll'), help='VPF is a safe option wherein you can contribute more than the PF ceiling of 12% that has been mandated by the government and VPF computed as percentage(%)')
    house_rent_allowance_metro_nonmetro = fields.Float(string='House Rent Allowance (%)', digits_compute=dp.get_precision('Payroll'), help='HRA is an allowance given by the employer to the employee for taking care of his rental or accommodation expenses for metro city it is 50% and for non metro 40%. \nHRA computed as percentage(%)')
...
class HrPayrollAdvice(models.Model):
    ...
    date = fields.Date(readonly=True, required=True, states={'draft': [('readonly', False)]}, help='Advice Date is used to search Payslips', default=_get_default_date)
    ...
    def confirm_sheet(self):
        ...
        for advice in self:
            ...
            advice.number = 'PAY' + '/' + advice_year + '/' + number
            advice.state = 'confirm'
...
class HrPayrollAdviceLine(models.Model):
    ...
    ifsc_code = fields.Char(string='IFSC Code', size=16)
    employee_id = fields.Many2one('hr.employee', string='Employee', required=True)
    bysal = fields.Float(string='By Salary', digits_compute=dp.get_precision('Payroll'))
    debit_credit = fields.Char(string='C/D', size=3, default='C')
    ...
    def onchange_employee_id(self, employee_id=False):
        ...
        if not employee_id:
            return {'value': res}
        employee = self.env['hr.employee'].browse(employee_id)[0]
...
class ResCompany(models.Model):
    ...
    dearness_allowance = fields.Boolean(string='Dearness Allowance', help='Check this box if your company provide Dearness Allowance to employee', default=True)
