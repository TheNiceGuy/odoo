...
class HrPayrollStructure(models.Model):
    ...
    def _get_parent(self):
        base_structure = self.env.ref('hr_payroll.structure_base', False)
        return base_structure and base_structure.id or False
...
class ContribRegister(models.Model):
...
class HrPayslip(models.Model):
    ...
    def refund_sheet(self):
        ...
        formview_id = formview_ref and formview_ref.id or False
        treeview_ref = self.env.ref('hr_payroll.view_hr_payslip_tree', False)
        treeview_id = treeview_ref and treeview_ref.id or False
        return {
    ...
    def compute_sheet(self):
        ...
        for payslip in self:
            number = payslip.number or Sequence.next_by_code('salary.slip')
    ...
    def get_worked_day_lines(self, contract_ids, date_from, date_to):
        ...
        for contract in self.env['hr.contract'].browse(contract_ids).filtered(lambda contract: contract.working_hours):
            ...
            for day in range(0, nb_of_days):
                working_hours_on_day = Calendar.working_hours_on_day(contract.working_hours, day_from + timedelta(days=day))
    ...
    def get_payslip_lines(self, contract_ids, payslip_id):
        ...
        class InputLine(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date=None):
                ...
                result = 0.0
                cr = self.env.cr
                cr.execute("SELECT sum(amount) as sum\
        ...
        class WorkedDays(BrowsableObject):
            ...
            def _sum(self, code, from_date, to_date=None):
                ...
                result = 0.0
                cr = self.env.cr
                cr.execute("SELECT sum(number_of_days) as number_of_days, sum(number_of_hours) as number_of_hours\
        ...
        class Payslips(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date=None):
                if to_date is None:
                    to_date = fields.Date.today()
                cr = self.env.cr
                cr.execute("SELECT sum(case when hp.credit_note = False then (pl.total) else (-pl.total) end)\
