    ...
    def _get_lines(self, dispatchmode=False, extendedmode=False):
        ...
        for inv_line in invoicelines:
            ...
            if inv_line.invoice_id.type in ('in_invoice', 'in_refund'):
                #comes from purchase
                poline = self.env['purchase.order.line'].search([('invoice_lines', 'in', inv_line.id)], limit=1)
                if poline:
                    location = self.env['stock.location'].browse(poline.order_id._get_destination_location())
                ...
                #comes from sales
                soline = self.env['sale.order.line'].search([('invoice_lines', 'in', inv_line.id)], limit=1)
                if soline:
                    saleorder = soline.order_id
