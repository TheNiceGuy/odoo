...
class StockBarcodeController(http.Controller):
    ...
    def main_menu(self, barcode, **kw):
        ...
        """
        try_change_attendance = self.try_to_change_attendance(barcode)
    ...
    def try_to_change_attendance(self, barcode):
        ...
        if corresponding_employee:
            if corresponding_employee.user_id:
                employee_check = corresponding_employee.sudo(corresponding_employee.user_id.id).attendance_action_change()
            else:
                employee_check = corresponding_employee.sudo().attendance_action_change()
