...
class AccountInvoice(models.Model):
    ...
    def create(self, vals):
        ...
        if self.env.context.get('type') == 'out_invoice' and not reference_type:
            # fallback on default communication type for partner
            partner = self.env['res.partner'].browse(vals['partner_id'])
            reference_type = partner.out_inv_comm_type
            if reference_type == 'bba':
                reference = self.generate_bbacomm(self.env.context['type'], reference_type, partner.id, '')['value']['reference']
            vals.update({
