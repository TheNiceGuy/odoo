...
class ProductUom(models.Model):
    ...
    def _compute_price(self, price, to_uom_id=False):
        if (not self or not price or not to_uom_id
...
class ProductCategory(models.Model):
    ...
    def _compute_product_count(self):
        prod_templates = self.env['product.template'].read_group([('categ_id', 'in', self.ids)], ['categ_id'], ['categ_id'])
        mapped_data = dict([(template['categ_id'][0], template['categ_id_count']) for template in prod_templates])
    ...
    product_count = fields.Integer(compute='_compute_product_count', help="The number of products under this category (Does not consider the children categories)")
...
class ProductTemplate(models.Model):
    ...
    type = fields.Selection('_get_product_template_type', string='Product Type', required=True, default='consu',
...
class ProductProduct(models.Model):
    ...
    def _get_pricelist_items(self):
        ...
        for product in self:
            items = ProductPricelistItem.search(['|', ('product_id', '=', product.id), ('product_tmpl_id', '=', product.product_tmpl_id.id)])
    ...
    pricelist_item_ids = fields.Many2many('product.pricelist.item', _compute='_get_pricelist_items', string='Pricelist Items')
...
class ProductSupplierinfo(models.Model):
    ...
    sequence = fields.Integer(help="Assigns the priority to the list of product vendor.", default=True)
