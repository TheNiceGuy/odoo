...
class WebsiteCrmPartnerAssign(WebsitePartnerPage):
    ...
    def partners(self, country=None, grade=None, page=0, **post):
        ...
        if not country and not country_all:
            ...
            if country_code:
                country_ids = Country.search([('code', '=', country_code)])
                if country_ids:
                    country = Country.browse(country_ids[0])
        if country:
        ...
        partners = Partner.sudo().search(base_partner_domain, order="grade_id DESC, display_name ASC")  # todo in trunk: order="grade_id DESC, implemented_count DESC", offset=pager['offset'], limit=self._references_per_page
        # remove me in trunk
        ...
        if partner_id:
            ...
            if partner.exists() and (partner.website_published or is_website_publisher):
                values = {
                    ...
                    'current_country': current_country if current_country else None
                }
