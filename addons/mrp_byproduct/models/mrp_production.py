    ...
    def _generate_moves(self):
        ...
        for production in self.filtered(lambda p: p.bom_id):
            ...
            for sub_product in production.bom_id.subproduct_ids:
                product_uom_factor = self.env['product.uom']._compute_qty(production.product_uom_id.id, production.product_qty, production.bom_id.product_uom_id.id)
                ...
                data = {
                    ...
                    'location_dest_id': self.location_destination().id,#production.location_dest_id.id,
