...
class ResourceCalendar(models.Model):
    ...
    def interval_remove_leaves(self, interval, leave_intervals):
        ...
        if leave_intervals is None:
            leave_intervals = []
        intervals = []
        leave_intervals = self._model.interval_clean(leave_intervals)
    ...
    def get_attendances_for_weekday(self, date):
        ...
        attenances = self.env['resource.calendar.attendance']
    ...
    def _schedule_hours(self, hours, day_dt=None,
        ...
        while float_compare(remaining_hours, 0.0, precision_digits=2) in (1, 0) and iterations < 1000:
            ...
            if not self and not working_intervals:  # no calendar -> consider working 8 hours
                ...
                new_working_intervals = self._model.interval_schedule_hours(working_intervals, remaining_hours, not backwards)
                if backwards:
    ...
    def working_hours_on_day(self, cr, uid, resource_calendar_id, day, context=None):
        ...
        resource_calendar_id = self.browse(cr, uid, resource_calendar_id.id, context=context)
        return ResourceCalendar.working_hours_on_day(resource_calendar_id, day)
...
class ResourceResource(models.Model):
    ...
    name = fields.Char(required=True)
    code = fields.Char(size=16, copy=False)
    ...
    time_efficiency = fields.Float(string='Efficiency Factor', size=8, required=True, default=1,
...
class ResourceCalendarLeaves(models.Model):
    ...
    def check_dates(self):
        if self.filtered(lambda leave: leave.date_from and leave.date_to and leave.date_from > leave.date_to):
