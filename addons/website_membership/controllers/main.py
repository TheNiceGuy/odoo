...
class WebsiteMembership(http.Controller):
    ...
    def members(self, membership_id=None, country_name=None, country_id=0, page=1, **post):
        ...
        if membership_id and membership_id != 'free':
            base_line_domain.append(('membership_id', '=', int(membership_id)))
        ...
        if membership_id != 'free':
            membership_line = MembershipLine.sudo().search(base_line_domain)
            country_domain = [('member_lines', 'in', membership_line.ids)]
        ...
        countries = Sudo_Partner.read_group(country_domain + [("website_published", "=", True)], ["id", "country_id"],
            groupby="country_id", orderby="country_id")
        countries_total = sum(country_dict['country_id_count'] for country_dict in countries)

        line_domain = base_line_domain
        if country_id:
            line_domain.append(('partner.country_id', '=', country_id))
            current_country = request.env['res.country'].browse(country_id)
        ...
        memberships = request.env['product.product'].search([('membership', '=', True)], order="website_sequence")
        ...
        membership_lines = MembershipLine
        if membership_id != 'free':
            count_members = MembershipLine.search_count(line_domain)
        ...
        if request.env.ref('website_membership.opt_index_google_map').customize_show:
            membership_line = MembershipLine.search(line_domain)
            google_map_partner_ids = membership_line.get_published_companies(limit=2000)
        ...
        free_partner_ids = Sudo_Partner.search(search_domain).ids
        memberships_data = []
        ...
        partners = {p.id: p for p in Sudo_Partner.browse(page_partner_ids)}
